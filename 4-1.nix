let
  ...
  get = y: x:
    ...
    then 0
    else lib.elemAt (lib.elemAt nums y) x;

  coordinates = lib.crossLists (y: x: {inherit x y;}) [(lib.range 0 (lengthY - 1)) (lib.range 0 (lengthX - 1))];
  ...
  count = y: x:
    ...
    + (get (y + 1) (x + 1));
in
  lib.foldl lib.add 0 (map (pos:
    if (count pos.y pos.x) < 4
    then 1
    else 0)
  rolls)
