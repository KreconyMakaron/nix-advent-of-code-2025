let
  ...
  isValid = x: lib.foldl' (prev: pref: prev || (isValidForPrefix x pref)) false (lib.range 1 ((lib.stringLength x) - 1));

  sumRange = x: y:
    lib.foldl' (prev: num:
      prev
      + (
        if (isValid (builtins.toString num))
        then num
        else 0
      ))
    0 (lib.range (lib.toIntBase10 x) (lib.toIntBase10 y));
in
