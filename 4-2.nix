let
  ...
  nums = map (row: row.row) (
    lib.zipListsWith (y: row: {
      inherit y;
      row = lib.zipListsWith (symbol: x: {
        inherit x y;
        sym = if symbol == "@" then 1 else 0;
      }) row (lib.range 0 (lengthX - 1));
    }) (lib.range 0 (lengthY - 1)) symbols
  );

  get =
    y: x: r:
    if x < 0 || y < 0 || x >= lengthX || y >= lengthY then
      {
        inherit x y;
        sym = 0;
      }
    else
      lib.elemAt (lib.elemAt r y) x;
  ...
  countRolls =
    r: lib.foldl lib.add 0 (map (row: lib.length (lib.filter (pos: accessible pos r) row)) r);
  removeRolls =
    r:
    map (
      row:
...
lib.fix (
  ...
  let
    next = removeRolls r;
  in
  if next == r then countRolls r else (countRolls r) + (self next)
) nums
